...
const Addnote = () => {
  ...
  const onchange = (e) => {
    // niche me hm spreadoperator ka use krrhe hai, iska matlb ki jo v kch note object ka andar me hai, wo sb rhe....lekin uske aage, jo v kch likha hua hai, wo sb note k andar overwrite ho jaye...

    setNote({ ...note, [e.target.name]: e.target.value }); // e.target tab likhte hai jb kisi textarea se koi writing extract krna hota hai--- yaha pe writing ko extract krke 'name' k andar set kr de rhe hai---F
  };
  const handleclick = (e) => {
    e.preventDefault();
